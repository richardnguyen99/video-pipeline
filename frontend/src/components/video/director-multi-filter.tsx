...
interface DirectorMultiFilterProps {
  selected: number[];
...
export function DirectorMultiFilter({ selected, onChange, container, triggerClassName }: DirectorMultiFilterProps) {
  ...
  const detailQueries = useQueries({
    queries: idsNeedingDetail.map((directorId) => ({
      ...directorDetailQueryOptions(directorId),
      staleTime: 5 * 60_000,
    })),
  });
  ...
  for (const result of detailQueries) {
    ...
    const entity = mapDirectorDetailToNamedEntity(result.data, locale);
    ...
    <DropdownMenu open={open} onOpenChange={handleOpenChange}>
      ...
      >
        <div className="flex flex-col gap-2 border-b border-border p-2">
          <DropdownMenuGroup>
            <DropdownMenuLabel className="px-0 py-0">Director (OR)</DropdownMenuLabel>
          </DropdownMenuGroup>
          <Input
            value={query}
