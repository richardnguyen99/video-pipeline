...
interface DirectorSingleFilterProps {
...
export function DirectorSingleFilter({ value, onChange, container, triggerClassName }: DirectorSingleFilterProps) {
  const locale = DEFAULT_DIRECTOR_LOCALE;
  ...
  const { data, fetchNextPage, hasNextPage, isFetchingNextPage, isFetching, isPending } = useInfiniteQuery(
    directorFilterInfiniteOptions(debouncedQuery || undefined, locale),
  );

  const options = useMemo(() => flattenDirectorFilterPages(data?.pages), [data?.pages]);
  ...
  const detailQuery = useQuery({
    ...directorDetailQueryOptions(value ?? 0),
    enabled: needsDetail,
    staleTime: 5 * 60_000,
  });

  const detailName =
    detailQuery.data != null ? mapDirectorDetailToNamedEntity(detailQuery.data, locale).name : undefined;
  ...
  const triggerLabel = value != null ? resolveName(value) : "Director";
    ...
    <DropdownMenu open={open} onOpenChange={handleOpenChange}>
      ...
      >
        <div className="flex flex-col gap-2 border-b border-border p-2">
          <DropdownMenuGroup>
            <DropdownMenuLabel className="px-0 py-0">Director</DropdownMenuLabel>
          </DropdownMenuGroup>
          <Input
            value={query}
