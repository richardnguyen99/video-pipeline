...
interface LabelMultiFilterProps {
  selected: number[];
...
export function LabelMultiFilter({ selected, onChange, container, triggerClassName }: LabelMultiFilterProps) {
  ...
  const detailQueries = useQueries({
    queries: idsNeedingDetail.map((labelId) => ({
      ...labelDetailQueryOptions(labelId),
      staleTime: 5 * 60_000,
    })),
  });
  ...
  for (const result of detailQueries) {
    ...
    const entity = mapLabelDetailToNamedEntity(result.data, locale);
