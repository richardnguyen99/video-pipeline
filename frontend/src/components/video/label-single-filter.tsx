...
interface LabelSingleFilterProps {
...
export function LabelSingleFilter({ value, onChange, container, triggerClassName }: LabelSingleFilterProps) {
  const locale = DEFAULT_LABEL_LOCALE;
  ...
  const { data, fetchNextPage, hasNextPage, isFetchingNextPage, isFetching, isPending } = useInfiniteQuery(
    labelFilterInfiniteOptions(debouncedQuery || undefined, locale),
  );

  const options = useMemo(() => flattenLabelFilterPages(data?.pages), [data?.pages]);
  ...
  const detailQuery = useQuery({
    ...labelDetailQueryOptions(value ?? 0),
    enabled: needsDetail,
    staleTime: 5 * 60_000,
  });

  const detailName = detailQuery.data != null ? mapLabelDetailToNamedEntity(detailQuery.data, locale).name : undefined;
  ...
  const triggerLabel = value != null ? resolveName(value) : "Label";
