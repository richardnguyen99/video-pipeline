...
interface MakerSingleFilterProps {
...
export function MakerSingleFilter({ value, onChange, container, triggerClassName }: MakerSingleFilterProps) {
  ...
  const detailQuery = useQuery({
    ...makerDetailQueryOptions(value ?? 0),
    enabled: needsDetail,
    staleTime: 5 * 60_000,
  });

  const detailName = detailQuery.data != null ? mapMakerDetailToNamedEntity(detailQuery.data, locale).name : undefined;
  ...
  const triggerLabel = value != null ? resolveName(value) : "Maker";
    ...
    <DropdownMenu open={open} onOpenChange={handleOpenChange}>
      ...
      >
        <div className="flex flex-col gap-2 border-b border-border p-2">
          <DropdownMenuGroup>
            <DropdownMenuLabel className="px-0 py-0">Maker</DropdownMenuLabel>
          </DropdownMenuGroup>
          <Input
            value={query}
