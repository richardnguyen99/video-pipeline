...
interface SeriesMultiFilterProps {
  selected: number[];
...
export function SeriesMultiFilter({ selected, onChange, container, triggerClassName }: SeriesMultiFilterProps) {
  ...
  const detailQueries = useQueries({
    queries: idsNeedingDetail.map((seriesId) => ({
      ...seriesDetailQueryOptions(seriesId),
      staleTime: 5 * 60_000,
    })),
  });
  ...
  for (const result of detailQueries) {
    ...
    const entity = mapSeriesDetailToNamedEntity(result.data, locale);
    ...
    <DropdownMenu open={open} onOpenChange={handleOpenChange}>
      ...
      >
        <div className="flex flex-col gap-2 border-b border-border p-2">
          <DropdownMenuGroup>
            <DropdownMenuLabel className="px-0 py-0">Series (OR)</DropdownMenuLabel>
          </DropdownMenuGroup>
          <Input
            value={query}
